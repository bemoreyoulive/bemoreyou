...
const afContentBobby = [
  ...
  },
  {
    ...
    postCopy: "A client sends in their bank statements in October.\n\nIn January, Bobby sends a request for the bank statements.\n\nThe client replies: \"I sent those three months ago.\"\n\nAnd Bobby has no answer — because there's no system tracking what was already received.\n\nThis happens when documents arrive by email over the course of a year and nobody has a reliable way to know what's already in. So at year-end, the request goes out again. For everything.\n\nIt's not a big deal once. It's a relationship problem when it keeps happening.\n\nYour clients remember. Even when you don't.\n\nThis is the kind of gap I fix — not glamorous, but it's the gap that quietly erodes trust.",
  ...
  },
  {
    ...
    postCopy: "When you were doing all the client work yourself, you knew exactly what had been answered and what hadn't.\n\nNow you've got a team. Client managers. Portfolios assigned. Responsibility delegated.\n\nAnd no reliable way to know if a client email sat unanswered for four days.\n\nYou don't want to read everyone's inbox. You shouldn't have to.\n\nBut right now, the first time you find out a response was missed is when the client tells you.\n\nThat's not a management process. That's hoping for the best.\n\nA weekly summary — which staff, which clients, what's overdue — takes about twenty minutes to set up. And it means Monday morning starts with facts, not assumptions.",
      ...
      <div style={{ maxWidth: 1160, margin: "0 auto", padding: "48px 36px" }}>
        ...
        {activeTab === "content" && (
          <div>
            ...
            {contentSubTab === "story" && afContentStory.map((idea, i) => <IdeaCard key={i} idea={idea} slug={slug} />)}
