...
export async function POST(req: NextRequest) {
  const { type, clientName, detail } = await req.json();
    ...
    html = `
      <div style="font-family: Inter, sans-serif; max-width: 560px; margin: 0 auto; padding: 40px 32px; background: #F5F1EC; border-radius: 4px;">
        ...
        <div style="background: #fff; border: 1px solid #E0DBD3; border-radius: 3px; padding: 20px 24px; margin: 0 0 24px;">
          <p style="font-size: 14px; color: #1C1C1C; margin: 0; line-height: 1.8; white-space: pre-wrap;">${detail}</p>
        </div>
        <p style="font-size: 13px; color: #7A746E; margin: 0;">Log in to <a href="https://bemoreyoulive.com/admin" style="color: #E8521C;">bemoreyoulive.com</a> to view the submission.</p>
      </div>
    `;
  }
