...
export async function POST(req: NextRequest) {
  // Verify the calling user is Ben by reading the Supabase session from cookies
  ...
  // Look up the client's email and name from client_profiles
...
<body style="margin: 0; padding: 0; background: #F5F1EC; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;">
  <div style="max-width: 600px; margin: 40px auto; background: #F5F1EC; padding: 0 20px 40px;">
    ...
    <div style="padding: 28px 0 24px;">
      ...
      <p style="font-size: 10px; font-weight: 700; letter-spacing: 0.18em; text-transform: uppercase; color: #7A746E; margin: 2px 0 0;">Update from Ben</p>
    </div>
  ...
  try {
    await resend.emails.send({
      ...
      to: profile.email,
      subject: `Update from Ben — ${profile.name}`,
      html,
    });
